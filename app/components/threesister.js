...
export default function ThreeSister() {
  const canvasRef = useRef();

  return (
    <Canvas ref={canvasRef} shadows="basic" eventPrefix="client" camera={{ position: [0, 1.5, 14], fov: 45 }}>
      <fog attach="fog" args={['black', 0, 20]} />
      <pointLight position={[10, -10, -20]} intensity={10} />
      <pointLight position={[-10, -10, -20]} intensity={10} />
      <Model position={[0, -5.5, 3]} rotation={[0, -0.2, 0]} />
      <SoftShadows samples={3} />
      <CameraControls ref={canvasRef} minPolarAngle={0} maxPolarAngle={Math.PI / 2} minAzimuthAngle={-Math.PI / 2} maxAzimuthAngle={Math.PI / 2} />
    </Canvas>
  );
}
...
function Annotation({ children, ...props }) {
  return (
    <Html
      ...
      <div className="transition-all duration-500 ease-in-out shadow-white cursor-pointer outline-none border-none text-xs font-light bg-indian-red text-white px-2 py-10 rounded-full flex justify-center items-center gap-5" onClick={() => console.log('.')}>
        {children}
      </div>
    </Html>
  );
